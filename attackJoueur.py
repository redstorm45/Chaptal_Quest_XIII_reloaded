...
def attack(joueur,ennemis,boss):
    #vers le bas
    ...
    for i in range(len(boss)):
        posH = collision.hitbox(boss[i])
        posE = [boss[i].position[1],boss[i].position[2]]
        damJ = (joueur.dammage + joueur.lvl + joueur.degatarme)*exp(-boss[i].armure/300)
        d = (posE[0]-posJ[0])**2 + (posE[1]-posJ[1])**2
        if joueur.direction in [1,2]:
            if aportee(posJ,posH[0]) <4 or aportee(posJ,posH[1])<4 and abs(posE[0]-posJ[0]) <= -(posE[1]-posJ[1]):
                if joueur.capacite2 == "RDM":
                    capacite.capacite("RDM",joueur,boss[i])                    
                boss[i].hp = boss[i].hp - damJ
